    ...
    sync: function(method, collection, options) {
      ...
      location.fetch({
        success: function(location) {
          ...
          } else if ((location.get('state') || location.get('region_code')) && location.get('city')) {
            query += 'state=' + (location.get('state') || location.get('region_code')) + '&city=' + location.get('city');
          } else if (location.get('latitude') && location.get('longitude')) {
            query += '$where=within_box(location, ' + (location.get('latitude') + 0.25) + ', ' + (location.get('longitude') - 0.25) + ', ' + (location.get('latitude') - 0.25) + ', ' + (location.get('longitude') + 0.25) + ')';
          }
